...
/** Per-file 5MB; total 15MB so Gmail stays under ~25MB after base64. */
export const IOI_ATTACHMENT_MAX_BYTES = 5 * 1024 * 1024;
export const IOI_ATTACHMENT_MAX_TOTAL_BYTES = 15 * 1024 * 1024;
...
export function normalizeGmailAttachments(rawList) {
  ...
  const totalBytes = out.reduce((n, att) => n + att.data.length, 0);
  if (totalBytes > IOI_ATTACHMENT_MAX_TOTAL_BYTES) {
    const err = new Error(
      `Attachments can total at most ${Math.round(IOI_ATTACHMENT_MAX_TOTAL_BYTES / 1024 / 1024)}MB`
    );
    err.status = 400;
